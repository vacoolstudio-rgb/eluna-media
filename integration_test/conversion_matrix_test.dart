...
void main() {
  ...
  group('входные форматы, которые приложение обещает читать', () {
    ...
    // канала нет**. Так и задумано: молчаливо обрезанная фотография — не то,
    // о чём тест должен помалкивать.
    //
    // С 2026-08-16 половина написана на обеих: iOS декодирует через ImageIO,
    // Android — через `ImageDecoder` (API 28+). На Android ниже 28 системного
    // декодера HEIF нет вовсе, и тест там упадёт честно: обещание читать HEIC
    // на таком устройстве не выполняется.
      ...
      expect(w, _heicWidth,
          reason: oneTile
              ? 'пришла ровно одна плитка $w×$h — значит StillDecoder не '
                  'сработал и файл ушёл в FFmpeg как есть. Смотреть надо в '
                  'нативную половину канала `decodeStill`: она отказывает '
                  'закрыто, поэтому любой её сбой выглядит именно так. На '
                  'Android ниже API 28 это не сбой, а отсутствие системного '
                  'декодера HEIF. Не «почините тест» — почините платформу или '
                  'снимите обещание читать HEIC.'
